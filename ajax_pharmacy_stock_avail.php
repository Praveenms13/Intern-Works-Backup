...
$drug_namee = $_POST['drug_nameed'];

$drug_name_temp = new ManageUsers();
$drug_template = $drug_name_temp->listDirectQuery("SELECT * FROM drug_template WHERE drug_name = '$drug_namee'");
$array = $drug_template[0];
...
$drug_id = $_POST['drug_ingredient_id'];



$drug_ingredient_temp = new ManageUsers();
$drug_ing = $drug_ingredient_temp->listDirectQuery("SELECT  name FROM ingredients WHERE id = '$drug_id'");

$name = $drug_ing[0]['name'];
...
if (isset($_POST) && !empty($_POST)) {
    ...
    if ($action == "aboutDrug") {
        ...
        echo '<div class="row-fluid row-fluid-style" style="margin:0px;">';
        ...
        $drugSearchObj = null;
        echo '<table class="table table-nomargin table-mail" id="tbl-heading">
						<tr>
							<th>Drug</th>
							<th>Available Qty</th>
							<th>Manufacturer & Expiry</th>
							<th>Cost Per Unit</th>
							<th>Action</th>
						</tr>';
        if (is_array($drugSearchs)) {
            foreach ($drugSearchs as $drugSearch) {
                ...
                echo '<tr>
							<td  data-content="Drug"><p>' . $drugName . '</p><p>' . $drug_type . '</p><p>Batch: ' . $drug_batch . '</p><p>Strength: ' . $drug_strength . '</p><p>Inventory: ' . $inventory . '</p></td>
							<td  data-content="Qty"><span id="stockBalQty' . $drugStockId . '">' . $balance_qty . '</span></td>
							<td><p>Manufacturer:</p><p>' . $drug_manifaturer . '</p><p>' . $drug_manifaturer_date . '</p><p>Expiry:</p><p>' . $drug_expiry_date . '</p><p>' . $drug_expiry_period . '</p></td>
							<td data-content="Amt">' . $amount_per_tab . '</td>
							<td data-content="Action">
							<p>Qty: <input type="text" id="stockQty' . $drugStockId . '-' . $prescriptionMainid . '" value="" class="input-minix" onKeyup=bringPharmacyCalculateQty(event,' . $drugStockId . ',' . $prescriptionMainid . ') /><input type="hidden" id="prescriptionMainid' . $drugStockId . '-' . $prescriptionMainid . '" value="' . $prescriptionMainid . '" class="input-minix"  /></p>
							<p>Amt: <span id="stockCost' . $drugStockId . '-' . $prescriptionMainid . '"></span></p>';

                if (basename($_SERVER['HTTP_REFERER']) == "pharmacy_billing_add.php") {
                    echo '<p><input type="button" value="Add" class="btn btn-primary" onclick="bringPharmacyAddWalkinUser(' . $drugStockId . ',' . $prescriptionMainid . ')" style="margin-top:10px;" /></p>';
                } elseif (basename($_SERVER['HTTP_REFERER']) == "corp_pharma_billing_add.php") {
                    echo '<p><input type="button" value="Add" class="btn btn-primary" onclick="bringPharmacyAddWalkinUser(' . $drugStockId . ',' . $prescriptionMainid . ')" style="margin-top:10px;" /></p>';
                } else {
                    echo '<p><input type="button" value="Add" class="btn btn-primary" onclick="bringPharmacyAdd(' . $drugStockId . ',' . $prescriptionMainid . ')" style="margin-top:10px;" /></p>';
                }
                echo '</td>
						 </tr>';
            }
...
<?php
    ...
    if ($action == "stockSoldAdd" || $action == "stockSoldAddWalkinUser") {
        ...
        if ($balance_qty >= 0) {
            ...
            if (empty($_SESSION['ohc_loca'])) {
                $sql = "INSERT INTO `pharmacy_sold_stock_detail`(r_pharmacy_id,r_user_id,r_appuser_walkin_id,r_prescription_id,stock_generate_id,r_stock_id,qty,amount,discount,r_doctor_id,r_hosp_id,created_by,created_role,created_on) VALUES ('" . $sessionlocation_id . "','" . $user_id . "','" . $r_appuser_walkin_id . "','" . $prescriptionMainid . "','" . $stockGenerationId . "','" . $stockId . "','" . $stockQty . "','" . $amount . "','" . $discount . "','" . $doctor_id . "','" . $hosp_id . "','" . $tbs_userid . "','" . $tbs_role . "','" . $datetime . "')";
            } else {
                $sql = "INSERT INTO `pharmacy_sold_stock_detail`(r_pharmacy_id,r_user_id,r_appuser_walkin_id,r_prescription_id,stock_generate_id,r_stock_id,qty,amount,discount,r_doctor_id,created_by,created_role,created_on,ohc) VALUES ('" . $_SESSION['ohc_loca'] . "','" . $user_id . "','" . $r_appuser_walkin_id . "','" . $prescriptionMainid . "','" . $stockGenerationId . "','" . $stockId . "','" . $stockQty . "','" . $amount . "','" . $discount . "','" . $tbs_role . "','" . $tbs_userid . "','" . $tbs_role . "','" . $datetime . "','1')";
            }
			...
			LEFT OUTER JOIN `pharmacy_stock_detail` psd ON psd.id=pssd.r_stock_id
			WHERE pssd.stock_generate_id='" . $stockGenerationId . "' AND psd.r_pharmacy_id='" . $sessionlocation_id . "' AND psd.drug_expiry_date>=date(now()) AND psd.isactive=0";
    ...
    if ($action == "stockinViewDetails") {
        if (isset($_SESSION['stock_generate_id'])) {
			...
			LEFT OUTER JOIN `pharmacy_stock_detail` psd ON psd.id=pssd.r_stock_id
			WHERE psd.r_pharmacy_id='" . $sessionlocation_id . "' AND pssd.stock_generate_id='" . $stockGenerationId . "' AND psd.drug_expiry_date>=date(now()) AND psd.isactive=0";
            } else {
                $sql = "SELECT pssd.id,psd.drug_name,pssd.qty,pssd.amount,pssd.discount,pssd.r_stock_id FROM `pharmacy_sold_stock_detail` pssd
			LEFT OUTER JOIN `pharmacy_stock_detail` psd ON psd.id=pssd.r_stock_id
			WHERE psd.r_pharmacy_id='" . $_SESSION['ohc_loca'] . "' AND pssd.stock_generate_id='" . $stockGenerationId . "' AND psd.ohc='1' and psd.drug_expiry_date>=date(now()) AND psd.isactive=0";
            }
            $qtyGetStokLists = $qtyGetObj->listDirectQuery($sql);
            $qtyGetObj = null;
            echo '
			<div class="row-fluid row-fluid-style tablecart" style="margin:0px;">
<div class="span12" ><img src="' . $sitepath . 'img/export-to-csv.png" align="right"/></div>
            ...
            foreach ($qtyGetStokLists as $qtyGetStokList) {
                ...
                echo "<tr>
						...
						<td style=text-align:center><a onclick='stockOutDel(" . $stockId . ")'><img src='" . $sitepath . "/img/cancel.png' style='width:15px;' /></td>
						</tr>";
            ...
            if (basename($_SERVER['HTTP_REFERER']) == "pharmacy_billing_add.php") {
                ...
                echo '<div class="clear" style="float:right;"><input type="button" class="btn btn-primary" value="Continue Billing" onclick="stockOutGenerateBill(' . $stockGenerationId . ',\'stockOutGenerateBill\')" /></div>';
            }
            echo '</div>';
        } else {
    ...
    if ($action == "uploadXls") {
        ...
        if ($extension == "csv") {
            ...
            while (!feof($csvfile)) {
                ...
                $insert_csv = array();
                $id = $csv_array[0];
                $insert_csv['drug_template_id'] = trim($csv_array[1]) ? trim($csv_array[1]) : "0";
                $insert_csv['drug_name'] = trim($csv_array[2]) ? trim($csv_array[2]) : "0";
                $insert_csv['drug_ingredient'] = trim($csv_array[3]) ? trim($csv_array[3]) : "0";
                $insert_csv['schedule'] = trim($csv_array[4]) ? trim($csv_array[4]) : "0";
                $insert_csv['drug_manifaturer'] = trim($csv_array[5]) ? trim($csv_array[5]) : "0";
                $insert_csv['drug_batch'] = trim($csv_array[6]) ? trim($csv_array[6]) : "0";
                $insert_csv['drug_manifaturer_date'] = date("Y-m-d", strtotime($csv_array[7])) ? date("Y-m-d", strtotime($csv_array[7])) : "0";
                $insert_csv['drug_expiry_period'] = trim($csv_array[8]) ? trim($csv_array[8]) : "0";

                //Expiry Date Calculation
                if (trim($csv_array[9]) == "") {
                    if (is_numeric($csv_array[8])) {
                        $expiryPeriod = " + " . $csv_array[8] . " months";
                    } else {
                        $expiryPeriod = " + " . $csv_array[8];
                    }
                    $drugExpiryDate = date("Y-m-d", strtotime($insert_csv['drug_manifaturer_date'] . " " . $expiryPeriod));
                } else {
                    $drugExpiryDate = date("Y-m-d", strtotime($csv_array[9]));
                }
                $insert_csv['drug_expiry_date'] = $drugExpiryDate;

                $insert_csv['drug_type'] = trim($csv_array[10]) ? trim($csv_array[10]) : "0";
                $insert_csv['drug_strength'] = trim($csv_array[11]) ? trim($csv_array[11]) : "0";
                $insert_csv['quantity'] = trim($csv_array[12]) ? trim($csv_array[12]) : "0";
                $insert_csv['amount_per_strip'] = trim($csv_array[13]) ? trim($csv_array[13]) : "0";
                $insert_csv['amount_per_tab'] = ($csv_array[14] == "") ? (round((trim($csv_array[13]) / trim($csv_array[16])), 2)) : trim($csv_array[14]);
                $insert_csv['unit_per_tab'] = trim($csv_array[15]) ? trim($csv_array[15]) : "0";
                $insert_csv['tablet_qty_strip'] = trim($csv_array[16]) ? trim($csv_array[16]) : "0";
                $insert_csv['discount'] = trim($csv_array[17]) ? trim($csv_array[17]) : "0";
                $insert_csv['ohc'] = trim($csv_array[18]) ? trim($csv_array[18]) : "0";
            ...
            while (!feof($csvfile)) {
                ...
                if (trim($csv_array[8]) == "") {
                    if (is_numeric($csv_array[7])) {
                        $expiryPeriod = " + " . $csv_array[7] . " months";
                    } else {
                        $expiryPeriod = " + " . $csv_array[7];
                    }
                    $drugExpiryDate = date("Y-m-d", strtotime($insert_csv['drug_manifaturer_date'] . " " . $expiryPeriod));
                } else {
    ...
    if ($action == "existUploadXls") {
		...
		LEFT OUTER JOIN `master_corporate_user` mpu ON mpu.id=psd.created_by
		WHERE  psd.r_pharmacy_id='" . $_SESSION['ohc_loca'] . "' and psd.ohc='1' AND psd.created_type=2 GROUP BY psd.upload_generate_id order by psd.created_on desc";
        }
