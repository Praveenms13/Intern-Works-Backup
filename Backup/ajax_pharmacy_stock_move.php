...
if($searchValue!=""){
		...
		if(empty($_SESSION['ohc_loca'])){
		    ...
		    // phrmcy_id
			$whr.=" AND psd.r_pharmacy_id='".$_SESSION['ohc_loca']."' AND psd.ohc='1' and psd.phar_ids='".$_SESSION['phrma_ids']."' AND psd.drug_expiry_date > '$tday' ";
...
$sels=(!empty($_SESSION['ohc_loca']))?$_SESSION['ohc_loca']:$_SESSION['currentlocation_id'];
		$stopissues= $getSqlData->listDirectQuery("select reminder_issue from master_pharmacy where id='".$sels."' ");
		...
		$getSqlData = null;
			...
			//echo $today."<".$expiryDateCheck."&&".$expiryDateCheck."<".$stopiss."</br>";
						if(strtotime($today)>=strtotime($expiryDateCheck) || $isactive=="0"){$style="color:#f00;";}
			...
			if(strtotime($today)<strtotime($expiryDateCheck)){
...
<div style="width:110px;float:left;"><input type="number" style="width:50px;" name="mqty"
 max="'.$listingMaintenace['balance_quantity'].'" required min="1" /></div><div style="float:left;"><select required name="mphar"><option value=""> Select Pharmacy</option>';
	...
	//<tr><td style="border-bottom: 1px #ccc Dashed;"></td><td colspan="6" style="border-bottom: 1px #ccc Dashed;"><i>';
...
echo 'History</p><p>Added '.$added.'</p><p>Modified '.$modified.'</p> </label></td>
				...
				</table></div>|*|*|*|*|<?php echo $pageNo; ?>|*|*|*|*|<?php echo $startLimit;?>|*|*|*|*|<?php echo $endLimit; ?>|*|*|*|*|<?php echo $count; 
			} else {
