...
<div class="col-xl-3 col-lg-3 d-none d-lg-block float-left "><h6 style="font-weight:bold;color:#<? echo $_SESSION['tclr'] ?>;"><? echo $userNameMenus."-".$Gtpharmacy[0]['name']; ?></h6> </div>
...
<select  name="phid" id="phid" class='uservalue a1 select2-me input-large w-100 nomargin texttransformcap checkvalid phhid' >
										...
										$phids=$pid[0];
										 ...
										 $phaname=$AppointmentDetail->listDirectQuery("SELECT name,id from ohc_pharmay where id in (".$Appointment[0]['phrmacy_id'].")"); 
										 if($phaname!=0){
				...
				else{  echo '<option value="'.$phid.'" >'. $pharnames.'</option>';}
				}			
	...
	var uservalue=$('#uservalue').val(); 
 	...
 	var dataString={uservalue:uservalue,phrid:phid,action:action};
 	$.ajax
		({ 
			type: "POST",
...
if (stockdata!="") {
	window.onload = function(){ restock(); }
}
function restock(){
		var pid='stock';
		$.fn.colorbox({href:"restockreview.php?pid="+pid,iframe:true, open:true, innerWidth:700, innerHeight:500,onOpen:function(){$('#cboxContent').removeClass("cboxContentCls");}});  
	 }
</script>