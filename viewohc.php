...
} else if ($favoritesCheck[0]['countfavorites'] > 0 && $fav_pharmacy != "") {
...
if ($favoritesCheck[0]['countfavorites'] == 0 && $fav_lab != "") {
    ...
    $favoritesAddCheckObj = null;
} else if ($favoritesCheck[0]['countfavorites'] > 0 && $fav_lab != "") {
    $favoritesUpdateCheckObj = new ManageUsers();
        ...
        foreach ($drugname as $val) {
            ...
            if (!empty($hiddendrugname[$q])) {
                ...
                if ($rowval == 0) {
                    ...
                    $prescriptionDetailId = $Prescriptions->addohPrescriptionDetails($SqlPresID, $drugnamevalue, $duration[$q], $early_morning[$q], $morning[$q], $late_morning[$q], $afternoon[$q], $late_afternoon[$q], $evening[$q], $night[$q], $late_night[$q], $drugtypevalue, $drugintakecondition[$q], $remarks[$q], $type);
                    unset($drugnamevalue);
                    unset($drugtypevalue);
...
if ($_POST['popUp'] == "1") { ?>
    ...
    if ($ohcs == '1' && !empty($case_id)) { ?>
        <script>
            document.location.href = "add-registry.php?id=<? echo $case_id; ?>";
        </script>
    <?    } else if ($saveRdraft == "Save" || $saveRdraft == "AgainDraftSavePresc" || $saveRdraft == "TemplateSave") { ?>
        <script>
