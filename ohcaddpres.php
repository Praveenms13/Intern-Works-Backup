...
if ($saveRdraft != "TemplateOnly" && $username != "") {
    if ($saveRdraft == "DraftPresc" || $saveRdraft == "DraftPrescs" || $saveRdraft == "CopyPresc" || $saveRdraft == "AgainDraftPresc" || $saveRdraft == "AgainDraftSavePresc") {
        ...
        foreach ($outsideDrugname as $val) {
            $prescriptionDetailId = $Prescriptions->addohPrescriptionDetails($SqlPresID, $val, $outsideduration[$q], $early_morning[$q], $Outsidemorning[$q], $late_morning[$q], $Outsideafternoon[$q], $late_afternoon[$q], $Outsideevening[$q], $Outsidenight[$q], $late_night[$q], $_POST['Outsidehiddendrugtype'][$q], $drugintakecondition[$q], $remarks[$q], 2);
            $q = $q + 1;
        }
