...
function viewsubpharAjax(){
 	...
 	var action="subphar";
 	var dataString={uservalue:uservalue,subphid:subphid,action:action};
 	$.ajax
		({ 
			type: "POST",
...
LEFT OUTER JOIN `master_user_details` mud on mud.id=p.user_id 
 WHERE p.status='1' AND p.isdraft='0'  AND IFNULL(p.fav_pharmacy,0)!=0   and IFNULL(p.fav_pharmacy_order,0)!=2  ".$whrPres." GROUP BY p.id ORDER BY p.from_date DESC,p.id DESC limit 0,20";
   ...
   <div class="w-100 float-left p-2" style=" background-color:#<?php echo $_SESSION['tclr'] ?>; color:#fff; font-size:14px; margin-top: 10px;">
                ...
                foreach($prescriptiondetail as $listprescriptiondetail) {
                    ...
                    echo'<input type="hidden" name="uid" value="'.$user_id.'"/><input type="hidden" name="created_role" value="'.$created_role.'"/>';?>
									...
									<div class="prescription-tr-table <?php echo $bg; ?>" style="width: 100%; float: left; padding: 3px 0; border-bottom: 1px #ccc dashed;">
										<div class="prescription-td-table" style="width:3%; text-align: center !important;"><input type="hidden" name="presid" id="presid" value="<?php echo $id;?>" />
...
<?php if ($listprescriptiondetail['prescription_type'] == "1" || $listprescriptiondetail['prescription_type'] == "0") { ?>
<div class="prescription-td-table" style="width:8%;">
...
<?php
    ...
    $today = date('Y-m-d');
    $medicines = "SELECT psd.id,psd.drug_name,psd.r_pharmacy_id,(psd.quantity-IFNULL(SUM(pssd.qty),0)) as balance_quantity
				...
				WHERE  psd.isactive='1' and psd.phar_ids='".$phrid."' and psd.ohc='1'  and psd.drug_expiry_date > '$today' GROUP BY psd.id having psd.r_pharmacy_id='".$_SESSION['ohc_loca']."' AND ifnull(balance_quantity,0)>0  order by psd.drug_expiry_date,psd.id ASC";
...
$sql = "SELECT SUM(cur_availability) as total_availability FROM pharmacy_stock_detail 
	...
	// 			LEFT OUTER JOIN `pharmacy_sold_stock_detail` pssd ON pssd.r_stock_id = psd.id
	// 			LEFT OUTER JOIN `master_corporate` mp ON mp.id = pssd.r_pharmacy_id
	// 			WHERE  psd.isactive='1' and psd.phar_ids='".$phrid."' and psd.ohc='1' and psd.drug_name LIKE '".trim($listprescriptiondetail['drugs_name'])."'  and psd.drug_expiry_date > '$today' GROUP BY psd.id having psd.r_pharmacy_id='".$_SESSION['ohc_loca']."' AND ifnull(balance_quantity,0)>0  order by psd.drug_expiry_date,psd.id ASC";
    ...
    //         $arr[$dname][] = $list['balance_quantity'];
    //         $mekey = array_search($dname, $tabnm);


    //     }  ?> 
<input type="hidden" name="medc[]" value="<?php echo $mekey;?>"/>
</div>
<div class="prescription-td-table" style="width:8%;">
<?php
if($total_qty < $totmedcount) {
    ...
    if($mainphr[0]['id'] != $phrid && $total_qty != 0) {?>
<input type="number" name="quanty[]" id="quanty" min="1" max="<?echo $totmedcount;?>" style="width:40%;" > <?php } ?></div>
<?php } else { ?>
...
<?php if($mainphr[0]['id'] != $phrid) { ?>
<input type="submit" name="issuetab-<?echo $id; ?>" value="Issue Partly"  style="color:white;background-color:#0088cc;" class="btn btn-sm">
<input type="submit" name="close" value="Close Prescription"  style="color:white;background-color:#0088cc;" class="btn btn-sm">
<?php }
}
